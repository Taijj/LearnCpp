...
T parallel_sum(std::vector<T> &vec)
{
    ...
    std::array<T, NUM_THREADS> sums{};

    const auto slice = vec.size()/NUM_THREADS;
    auto start = vec.begin();
    for(std::uint32_t i = 0; i < NUM_THREADS; ++i)
    {
        bool isLast = i == (NUM_THREADS-1);
        auto end = isLast ? vec.end() : start + slice;
