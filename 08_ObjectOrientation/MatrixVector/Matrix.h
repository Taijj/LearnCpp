...
Matrix<T>::Matrix(const std::size_t rows, const std::size_t cols)
    : m_rows(rows), m_cols(cols), m_data(MatrixDataType(rows, std::vector<T>(cols, T{}))) // Default of T
{
}

template <typename T>
Matrix<T>::Matrix(const std::size_t rows, const std::size_t cols, const T value)
    : m_rows(rows), m_cols(cols), m_data(MatrixDataType(rows, std::vector<T>(cols, value)))
{
}

// TODO: Foreach, check if matrices have the same order (same columns/rows)
...
Matrix<T> Matrix<T>::operator*(const Matrix &rhs) const
{
    ...
    // I hav eno idea what this is, it was not explained in the course, but apparently
    // this is the algorithm to multiply matrices... Mathmatics man
